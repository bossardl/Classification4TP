...
def get_args():
    ...
    parser.add_argument('--model_type', type=str, default='base',
                        help='Model type in [base, small_model].')
    ...
    parser.add_argument('--method', type=str, default=None, 
                        choices=['undersampling', 'oversampling', 'None'],
                        help='Method to tackle imbalance of the majority class \
                            ["undersampling", "oversampling", None] (default: None).')
    
    parser.add_argument('--undersampling', type=float, default=None,
                        help='Percentage of undersampling of the majority class in [0,1] \
                            (required if --method is "undersampling").')
    
    parser.add_argument('--oversampling', type=float, default=None,
                        help='Percentage of oversampling of the minority class in [0,1] \
                            (required if --method is "oversampling").')
    ...
    if args.method == 'undersampling':
        if args.undersampling is None or not (0 < args.undersampling <= 1):
            print('Error: --undersampling must be specified and between 0 and 1 \
                   when --method is "undersampling".')
            sys.exit(1)
    ...
    # Check for --oversampling value if method is oversampling
        ...
        if args.oversampling is None or not (0 < args.oversampling <= 1):
            print('Error: --oversampling must be specified and between 0 and 1 \
                   when --method is "oversampling".')
            sys.exit(1)
